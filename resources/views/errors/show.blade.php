        ...
        <div>
            ...
            <div class="card p-0 relative">
                <table tabindex="0" class="data-table">
                    ...
                    <tbody tabindex="0">
                        @forelse (array_reverse($hits) as $hit)
                            <tr class="row outline-none" tabindex="0">
                                <td><p class="truncate m-0 w-48" title="{{ $hit['data']['userAgent'] ?? '' }}">{{ $hit['data']['userAgent'] ?? '' }}</p></td>
                                <td>{{ $hit['data']['ip'] ?? '' }}</td>
                                <td>{{ $hit['data']['referer'] ?? '' }}</td>
                                <td><time datetime="{{ \Illuminate\Support\Carbon::createFromTimestamp($hit['timestamp'])->toIso8601String() }}">{{ isset($hit['timestamp']) ? \Illuminate\Support\Carbon::createFromTimestamp($hit['timestamp'])->diffForHumans() : '' }}</time></td>
                            </tr>
                        @empty
                            <tr class="row outline-none" tabindex="0">
